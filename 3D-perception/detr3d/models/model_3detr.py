...
class Model3DETR(nn.Module):
    ...
    def __init__(
        ...
        # self.build_mlp_heads(dataset_config, decoder_dim, mlp_dropout) # no need to build prediction heads here
    ...
    def forward(self, inputs, encoder_only=False, return_features=True):
        ...
        if return_features:
            ...
            # if return_intermediate == True, decoder_features will be a list of intermediate outputs
            # -> decoder_features: (num_layers, num_queries, batch_size, decoder_dim) -> we have to take the last layer's output: decoder_features[-1]
            # if return_intermediate == False, decoder_features will be (num_queries, batch_size, decoder_dim)
            return {
                "encoder_features": enc_features,            # (npoints, B, C)  
                "decoder_features": decoder_features,          # (num_queries, B, C) (256, 1, 256)
            }
        else: # if return_immediate == False, we can't do detection
...
def build_decoder(args):
    ...
    decoder = TransformerDecoder(
        decoder_layer, num_layers=args.dec_nlayers, return_intermediate=False # True for intermediate outputs, False for only last layer -> for better efficiency
    )
    return decoder
